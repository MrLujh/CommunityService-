...
//    CGRect viewR = CGRectMake(0, 0, frame.size.width*2.0, frame.size.height*2.0);
    
    CGRect viewR = CGRectMake(10, 0, frame.size.width+20, frame.size.height * 2.0);//zhj

    self = [super initWithFrame:viewR];
    if (self) {
        self.backgroundColor = [UIColor clearColor];
        bgcolor_ = color;
        self.info_ = [[NSString alloc] initWithString:info];
...
//    float y = (rect.size.height - fontSize_.height) / 2.0;
    ...
    [self.info_ drawInRect:r withFont:[UIFont systemFontOfSize:kSGInfoAlert_fontSize] lineBreakMode:UILineBreakModeTailTruncation];
}
...
+ (void)showInfo:(NSString *)info 
    ...
    CGRect frame = CGRectMake(10, 0, size.width, size.height);
    ...
    alert.alpha = 1.0;
    [UIView commitAnimations];
    [alert performSelector:@selector(fadeAway) withObject:nil afterDelay:1.0f];
}
